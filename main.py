...
async def send_morning_news():
    ...
    try:
        ...
        quote = "Сделай сегодня что-то великое! ✨"
    ...
    for user_id, city, bio in users_data:
        try:
            ...
            # --- Б. УМНАЯ НОВОСТЬ ОТ ИИ (Персонально) ---
            # Если био пустое, ставим тему по умолчанию
            user_interests = bio if bio else "Технологии и наука"

            # Формируем хитрый промпт
            ai_prompt = (
                    f"Пользователь интересуется темой: '{user_interests}'."
                    f"Напиши один короткий интересный факт, совет или мини-новость по этой теме."
                    f"Уложись в 2-3 предложения. Пиши позитивно."
            )

            try:
                # Спрашиваем ИИ (это может занять 2-3 секунды на каждого юзера)
                ai_news = await get_chat_response(ai_prompt)
            except:
                ai_news = "Сегодня отличный день, чтобы узнать что-то новое! ✨"
...
async def main():
    ...
    scheduler.add_job(send_morning_news, "cron", hour=5, minute=0)
